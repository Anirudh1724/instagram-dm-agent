...
def _verify_signature(payload: bytes, signature: str) -> bool:
    ...
    # TODO: Remove this after fixing the app secret
    logger.warning("signature_verification_bypassed", message="TEMPORARY - remove after testing")
    return True
    ...
    if not is_valid:
        logger.warning("signature_mismatch", 
                      received_signature=signature[:30] + "..." if signature else "NONE",
                      expected_signature=expected_full[:30] + "...",
                      secret_length=len(settings.meta_app_secret),
